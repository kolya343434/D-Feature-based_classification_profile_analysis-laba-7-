...
В работе реализована классификация (распознавание) символов по **нормализованным признакам** и (опционально) сравнение **профилей**.

**Задание (кратко):**
1. Реализовать меру близости символов по признакам: **масса**, **центр тяжести**, **осевые моменты инерции**. Метрика — евклидово расстояние в *n*-мерном пространстве нормализованных признаков. Нулевое расстояние соответствует единичной мере близости.
2. Для каждого найденного символа строки рассчитать близость со всеми символами выбранного алфавита → получить *N* гипотез на символ.
3. Вывести гипотезы в файл (в *i*-ой строке — гипотезы для *i*-ого символа), отсортировать по убыванию близости.
4. Вывести лучшие гипотезы (первый столбец) строкой и сравнить с исходной строкой.
5. Посчитать количество ошибок и долю (%) верно распознанных символов.
6. Эксперимент: сгенерировать изображение исходной строки со шрифтом другого размера (на несколько пунктов), распознать и сравнить.
7. (Для магистров) Дополнить меру близости сравнением профилей метрикой Левенштейна.
